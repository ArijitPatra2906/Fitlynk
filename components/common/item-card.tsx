...
export function ItemCard({
      ...
      <div className="flex-1 min-w-0">
        ...
        {(metadata || secondaryMetadata) && (
          <div className="sm:hidden mt-1 flex items-center gap-2">
            ...
            {secondaryMetadata && (
              <div className="text-[11px] text-gray-500 truncate">{secondaryMetadata}</div>
            )}
          </div>
        )}
      </div>
  ...
  return (
    <div className="flex items-center gap-2.5 sm:gap-3.5 p-3 sm:p-4 bg-[#131520] border border-white/5 rounded-2xl mb-2.5">
      {href ? (
        <Link href={href} className="flex items-center gap-2.5 sm:gap-3.5 flex-1 min-w-0">
          {content}
        </Link>
      ) : (
        <div className="flex items-center gap-2.5 sm:gap-3.5 flex-1 min-w-0">{content}</div>
      )}
      {showActions && (
