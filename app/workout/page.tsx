...
function WorkoutPageContent() {
    ...
    const initWorkout = async () => {
      try {
        ...
        if (templateId) {
          ...
          const templateRes = await apiClient.get(`/api/workouts/${templateId}`, token);
          if (templateRes.success) {
            ...
            const createRes = await apiClient.post('/api/workouts', newWorkout, token);
          ...
          const tempName = isTemplateMode ? "Untitled Template" : "Untitled Workout";
          const newWorkout: Workout = {
            name: tempName,
            started_at: new Date().toISOString(),
            exercises: [],
  ...
  const autoSaveWorkout = async (updatedExercises: ExerciseInWorkout[]) => {
    ...
    try {
      ...
      const exercisesForBackend = updatedExercises.map(ex => ({
        ...ex,
        exercise_id: typeof ex.exercise_id === 'object' ? ex.exercise_id._id : ex.exercise_id
      }));
  ...
  const finishWorkout = async () => {
    ...
    try {
      ...
      const res = await apiClient.put(`/api/workouts/${workout._id}`, updatedWorkout, token);
      if (res.success) {
  ...
  const handleAddExercise = async (exercise: Exercise) => {
    ...
    if (workout?._id) {
      try {
        const token = await getAuthToken();
        if (token) {
          const updatedExercisesForBackend = [...exercises.map(ex => ({
            ...ex,
            exercise_id: typeof ex.exercise_id === 'object' ? ex.exercise_id._id : ex.exercise_id
        ...
        {!isTemplateMode && (
          <div className="flex gap-2.5">
            <div className="flex-1 text-center bg-[#0B0D17] rounded-xl py-2">
              <div className="text-[14px] font-bold text-white">{stats.totalSets}</div>
              <div className="text-[10px] text-gray-400">Sets</div>
            </div>
            <div className="flex-1 text-center bg-[#0B0D17] rounded-xl py-2">
              <div className="text-[14px] font-bold text-white">{Math.round(stats.totalVolume)} kg</div>
                ...
                {exercise.sets.map((set, setIndex) => (
                  ...
                  >
                    ...
                    <input
                      type="number"
                      value={set.weight_kg}
                      onChange={(e) => updateSet(exerciseIndex, setIndex, 'weight_kg', Number(e.target.value))}
                    ...
                    >
                      {set.completed_at && <Icon name="check" size={16} color="white" strokeWidth={2.5} />}
                    </button>
                  </div>
                ))}
                <button
